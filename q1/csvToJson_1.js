...
fs.readFile('WDI_Data.csv', 'utf8', function(err, csv) {
  ...
  function compare(a,b) {
    var gdp1 = parseFloat(a["GDP_2005"]);
    var gdp2 = parseFloat(b["GDP_2005"]);
    if (gdp1 > gdp2)
      return -1;
    if (gdp1 < gdp2)
     return 1;
...
function processSampleFile(inputFile) {
  ...
  rl.on('close', function (line) {
    ...
    for(var i=0;i<15; ++i){
      ...
      var temp = parseFloat(resultf[i]["GDP_2005"]);
      obj["growth1"] = parseInt(temp);
      temp = parseFloat(resultf[i]["GNI_2005"]);
